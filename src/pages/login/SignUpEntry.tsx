...
const SignUpEntry: React.FC = () => {
  ...
  const renderRecommendedAgents = () => {
    if (!categoryAgents || categoryAgents.length === 0) {
      return (
        <Typography>おすすめのエージェントは見つかりませんでした。</Typography>
      );
    }
  ...
  return (
    <Box mt={4} display="flex" flexDirection="column" sx={{ width: "100%" }}>
      ...
      <TextField
        ...
        onChange={(e) =>
          dispatch(setUserField({ key: "email", value: e.target.value }))
        }
        sx={{ mb: 2, width: "50%" }}
      />
      ...
      <TextField
        select
        label="使用しているコミュニケーションツール"
        value={sliceUserInf.communicationTool}
        onChange={(e) =>
          dispatch(
            setUserField({ key: "communicationTool", value: e.target.value })
          )
        }
        fullWidth
        SelectProps={{
      ...
      <TextField
        ...
        onChange={(e) =>
          dispatch(
            setUserField({ key: "interestedCategory", value: e.target.value })
          )
        }
        fullWidth
        SelectProps={{
      ...
      <TextField
        ...
        onChange={(e) =>
          dispatch(
            setUserField({ key: "personalIssue", value: e.target.value })
          )
        }
        sx={{ mb: 2, width: "70%" }}
      />
      ...
      <Typography variant="h5" fontWeight="bold" gutterBottom>
        Myles.の使い方がひと目でわかる実例集です。
        <br />
        あなたに合った使い方を見つけて、実際に試してみましょう。
      </Typography>
      <Typography variant="h5" fontWeight="bold" mt={4} gutterBottom>
        “こんな使い方があったのか！”が見つかる場所。気になるパターンはすぐにお試しもできます。
        <br />
        業務でどう使えるか、活用例から発見してみてください。
      </Typography>
      ...
      <Box
        ...
        gap={4}
        mt={4}
        sx={{ overflowX: "auto", width: "100%" }} // 横スクロールも保険で入れる
      >
        <Box textAlign="center">
          <Typography variant="subtitle1" gutterBottom>
            お試しいただいたMyleをSlack、TEAMS上でご使用いただけます。
          </Typography>
          <img
            src="images/slackUseCase.png"
            alt="Mylesの使い方1"
